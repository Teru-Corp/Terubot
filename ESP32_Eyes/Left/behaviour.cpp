...
void behaviourActivate(String cmd) {
  // LEFT/RIGHT/UP/DOWN/CENTER override idle target immediately
  if (cmd == "LEFT") targetGazeX = -0.9f;
  if (cmd == "RIGHT") targetGazeX =  0.9f;
  if (cmd == "UP") targetGazeY = -0.9f;
  if (cmd == "DOWN") targetGazeY =  0.9f;
  if (cmd == "CENTER") {
    targetGazeX = 0.0f;   // center
    targetGazeY = 0.0f;
  }

  // If you want expressions (HAPPY/SAD/NEUTRAL/YES/NO) to return to idle:
  if (cmd == "HAPPY" || cmd == "SAD" || cmd == "NEUTRAL" || cmd == "YES" || cmd == "NO") {
...
void drawBaseEye(float cx, float cy, float m, float gazeX, float gazeY, int sideSign) {
  float R = 120.0f;
