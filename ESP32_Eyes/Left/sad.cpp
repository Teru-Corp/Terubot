...
void drawSadLid(float cx, float cy, float lidAmount, int sideSign) {
    float R = 120.0f;
    ...
    for (int x = -120; x <= 120; x++) {
        ...
        if (xf * xf <= R * R) {
            ...
            // Create stronger tilt - left side (negative x) droops MORE
            float tiltFactor = (-xf / 120.0f) * 35.0f;  // Increased from 15.0f to 35.0f for more dramatic tilt
