...
void processCommand(char cmd) {
  ...
  // C = Center gaze + Neutral expression
  if (cmd == 'C') {
    currentCommand = 'N';  // Neutral expression
    morphProgress = 0.0f;
    behaviourActivate('C');  // Center gaze
    return;
  }
  ...
  // H/S = Expression commands
  currentCommand = cmd;
  morphProgress = 0.0f;
  if (currentCommand == 'S') sadActivate();
  behaviourActivate('C');
}
...
void loop() {
  ...
  // Update gaze behavior (responds to L/R/U/D/C commands and idle wandering)
  behaviourUpdate(dt);
  ...
  if (currentCommand != 'S') {
    drawBaseEye(120.0f, 120.0f, lidAmount, gazeX, gazeY);
  } else {
    // For sad expression, draw eye without lid first, then add tilted sad lid
    drawBaseEye(120.0f, 120.0f, 0.0f, gazeX, gazeY);  // Draw with open eyes
    drawSadLid(120.0f, 120.0f, lidAmount);  // Draw tilted sad lid instead
  }
  ...
  if (currentCommand == 'H') {
    drawHappyExtras(120.0f, 120.0f, morphProgress);
  }
