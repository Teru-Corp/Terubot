...
void drawHappyExtras(float cx, float cy, float progress) {
  ...
  if (animationValue > 0.1f) {
    ...
    // Star 1 - Upper right (VERY BIG to small variation)
    float size1 = 8.0f + (poundEffect * 12.0f);
    drawStar(cx + 70.0f, cy - 70.0f, size1, COL_WHITE);
    
    // Star 2 - Upper left (offset phase for alternating blink)
    ...
    // Star 4 - Lower right (offset phase)
    ...
    // Star 5 - Lower left (offset phase) - bonus star for more sparkle
